...
### BUG-PD27 [D HARNESS, FIXED] DD4_CANCEL_OWN_CLICK_DISMISSAL — the probe's Cancel-position click landed on the re-rendered Done
- **Class**: D — the product held the PD24 contract perfectly; the harness probe defeated itself
- **Where**: `macos/scripts/exploratory-qa.sh` → the DD4 Cancel-during-upload probe
- **First red**: run 35352445690 (the final dataio source run, 15:08:15Z) — `[bug-P2] DATAIO_DD4_CANCEL_UPLOAD "the import did not complete after the during-upload Cancel click (neither completion nor an honest error was visible within 30s)"`
- **Root cause**: the one-row 98-byte upload completes in ~0.3s — by the probe's 0.7s-later click the footer had ALREADY re-rendered from [Cancel|Import Patients] to [Import Another|Done], so the Cancel-position click landed on 'Done' and dismissed the completed result panel the probe was about to OCR. The evidence proves the product behavior was CORRECT: the import COMMITTED (the API POST is in the log after the mark; the patient badge 13→14), the Cancel during flight was ineffective (the PD24 lock), and the dialog closed only via the legal complete-phase Done path
- **Fix**: the probe now accepts the API-log commit proof — if 'Import Successful' is not OCR-visible within 30s BUT the POST /api/patients/import hit the API since the mark AND the dialog is gone (no 'Need a template'), the commit is the proof (GREEN, with the actual mechanism recorded). Fail-closed preserved: a genuinely stuck dialog or a missing POST still P2s
- **Also recorded in the same run** (P3, product, recorded-not-stopping): DATAIO_IMPORT_NO_DEDUPE (two identical rows → imported=2, no duplicate detection) and DATAIO_IMPORT_BAD_DOB ('not-a-date' accepted verbatim — no import-side date validation) — next-round product backlog

### BUG-PD28 [D HARNESS, FIXED] DE6_EXPORT_SCHEMA_NEEDLE — the export check grepped the IMPORT template's schema
- **Class**: D — the exported file was PERFECT; the needle was wrong
- **Where**: `macos/scripts/exploratory-qa.sh` → the DE6 Export CSV verification
- **First red**: run 35355377514 (the final shard E, 14:49:48Z) — `[bug-P2] DESKTOP_EXPORT_CSV "the exported CSV exists but its header line is not the expected schema: First Name,Last Name,DOB,Phone,Email,Address,Notes,Document Count,Created Date"` — the message's tail IS the file's actual (correct!) header
...
### BUG-PD39 [D HARNESS, FIXED] MICRO_CAMERA_ENTRY_ROW_SKIP — the zero-document detail is SHORT; the coarse scroll leaped over the Action-Buttons row
- **Class**: D — harness-only (zero product change; the product renders the row unconditionally)
- **Where**: `macos/scripts/exploratory-qa.sh` → micro:camera, between docb_open_patient_docs and the 'Scan with Camera' click
- **First red**: run 35435788973 (micro:camera, 10:02Z) — `vclickc-open]: target 'Scan with Camera' NOT FOUND on screen`; the run's own captures show the coarse sweep (visits → clinical notes → the 'No documents yet' empty state) with the Action-Buttons row never inside any viewport; the failure capture (VLM-adjudicated) shows the empty state + its own 'Upload Your First Document'/'Scan' buttons filling the content area with the row above the fold
- **Root cause**: the BUG-PD21 class exactly — docb_open_patient_docs' 12-line bursts fire the keyboard assist (Page Down ≈ the full 768px viewport) every 3rd burst, and the camera fixture patient has ZERO documents, so the detail page is SHORT (identity → visits-empty → rx-empty → notes → the Action-Buttons row → the empty state ends the page): the assist leaped entirely over the ~40px row between OCR polls and stranded the view at the page bottom. The viewer-pdf shard of the SAME wave (1-document patient, same helper, same DMG) found 'Upload Files' after 2 bursts — the product renders and OCRs the row fine; only the zero-document geometry skips it
- **Fix**: the documented PD21 remedy — a FINE sweep (4-line steps, no keyboard assist) for the click target itself: `v_scroll_find "Scan with Camera" 8 no up 4 || v_scroll_find "Scan with Camera" 10 no down 4` (up first: at the bottom, the row sits directly above the empty state; down as the fallback). Scoped to micro:camera only — the shared helper and the other shards are untouched

### BUG-PD40 [D HARNESS, FIXED] GATEWAY_SKIP_SHARD_SHELL_PROOF — the tour offer dims 'Add Patient' out of the OCR for the TOUR_GATEWAY=skip shards
- **Where**: `macos/scripts/exploratory-qa.sh` → GATEWAY 6, the five post-submit shell waits (dashboard-after-enter-submit / -refill / -setup / -final-refill / -setup-final)
- **First red**: run 35435788973 (micro:tour-ar, 10:00Z) — `bug-P1 ACCOUNT_CREATION: the dashboard ('Add Patient') never appeared` with the failure capture showing the mounted shell: the tour offer card ('Welcome to MediVault', Step 1 of 20) + its spotlight overlay dimming the dashboard + the language prompt (both intentionally up for the shard — TOUR_GATEWAY=skip keeps them as the shard's own subject)
- **Root cause**: the offer auto-starts at the FIRST authenticated-shell mount; its spotlight dims the dashboard's 'Add Patient' out of the OCR until the shard's own tour battery dismisses it. The gateway's plain 'Add Patient' waits could only pass by winning the pre-offer mount race — the first flight (35385371196) won it; this run's storm/refill path mounted the offer before the final waits and lost it deterministically (the 90s wait's own captures read the dimmed chrome as garbled phonetics). NOT a product bug: the offer rendering + dimming is FEATURE C working as designed on a mounted shell
- **Fix**: `shell_mounted_ok` — the gateway shell-mounted proof accepts EITHER 'Add Patient' (every battery, unchanged) OR, for TOUR_GATEWAY=skip shards ONLY, the live offer needle 'Welcome to MediVault' (the offer exists solely on the authenticated shell, so it proves the same mount fact). The five g6 waits now use it; the refill/verification paths keep their own logic (the offer's card carries no 'Full Name', so no false refill can trigger)

### BUG-PD41 [D HARNESS, FIXED] MICRO_BULK_RESELECT_BADGE_GATE — the destructive leg was gated on an OCR-unreadable text-xs badge
- **Where**: `macos/scripts/exploratory-qa.sh` → micro:bulk-delete BD5 (the destructive gate)
- **First red**: run 35435788973 (micro:bulk-delete, 10:20Z) — `[bug-D] MICRO_BULK_RESELECT: the '2 selected' badge could not be confirmed … the delete is NOT taken` after a fully-successful deterministic re-select (Select All → Deselect All → Bravo + Delta row clicks, all hash-verified); the run then cascaded into BD7 with the cohort un-deleted
- **Root cause**: the floating bar's 'N selected' badge is `text-xs` (~12px) emerald-on-white — Apple Vision NEVER read it once in the entire run (bd2/bd4/bd5 all missed), though it rendered correctly (the bd5-two-selected capture reads '2 sele…'; VLM-adjudicated) and the product's confirmation dialog counted exactly 2 (bd3's 'Delete 2 Patients?' OCR-proven in the SAME run). The harness's OCR-only badge gate blocked its own destructive step
- **Fix**: the badge check is now a non-blocking corroboration probe; the authoritative count gate is the product's OWN confirmation-dialog title — the destructive leg opens the confirmation and verifies 'Delete 2 Patients?' there; a dialog count that is NOT 2 cancels immediately (`bd5-wrong-count-cancel`) and P1s MICRO_BULK_COUNT_GATE (a miscounted destructive confirmation would be a REAL product red); a never-appearing dialog keeps the honest P1. Wrong-pair safety stays with the P0 survivor gates + the gone checks (the post-delete world)

### BUG-PD42 [D HARNESS, FIXED] MICRO_BULK_FILTERED_SELECT_SCROLL — BD7's dashboard-flavored down-sweeps stranded the view at the page bottom, hiding the header's 'Select Patients'
- **Where**: `macos/scripts/exploratory-qa.sh` → micro:bulk-delete BD7 (post-search re-entry)
- **First red**: run 35435788973 (micro:bulk-delete, 10:21Z) — `[bug-P1] MICRO_BULK_FILTERED_SELECT: 'Select Patients' could not be entered in the filtered view` — the failure capture shows the filtered rows with the header above the fold after `v_scroll_find 'Recent Patients' 6 || v_scroll_find 'patients' 6` (12 down-bursts; both needles dashboard-flavored/absent on the patients page)
- **Root cause**: the 'Select Patients' button renders in the patients-list HEADER (next to the count badge) — the two down-sweeps pushed the viewport to the page bottom, exactly the BUG-PD21 class
- **Fix**: after the 'Bulk' search, `v_scroll_top` + the fine sweep (the PD21 idiom) for 'Select Patients' (down from the top, up as the fallback)

### BUG-PD43 [D HARNESS, FIXED] MICRO_SAVE_PDF_REPORT_ANCHOR — the banner name 'Micro Printest' systematically OCR'd as 'Micro Printers' (the BUG-PD36 fixture-name class)
- **Where**: `macos/scripts/exploratory-qa.sh` → dsk_click_banner_report (shared) + the micro:save-pdf MS2 / micro:print MP2 callers
- **First red**: run 35435788973 (micro:save-pdf, 10:25Z) — `[bug-P1] MICRO_SAVE_PDF_REPORT: the Generate Report (banner) control could not be activated` — `anchor 'Micro Printest' not on screen` although the failure capture shows the banner at the top with the name rendered (VLM reads 'Micro Printers' — the OCR's systematic misread; 55 'Printers' vs 2 'Printest' occurrences in the run's own OCR)
- **Root cause**: the fixture name's 'test' renders such that Apple Vision reads 'ters' at this size — the same class as PD36's 'View Pdftest'→'View Protest'. The banner anchor gated on the name alone
- **Fix**: dsk_click_banner_report takes an optional OCR-stable third anchor (the banner's own phone, '+1 555 0460' — the token the row click used successfully seconds earlier); the name stays primary, the phone is the fallback. The band math was already name-independent (detail_banner_row takes the topmost content line). Both micro callers now pass `$MICRO_PAT1_PHONE`
- **Note**: the same run's MS1 viewer-Print-icon ENV record is the documented WKWebView print-family gap (frozen print-shard class) — unchanged

### BUG-PD44 [D HARNESS, FIXED] TOUR_EN_ESC_SKIP_LADDER — mte4's skip was Escape-ONLY; the WKWebView key delivery failed from the step-7 scan-view spotlight context
- **Where**: `macos/scripts/exploratory-qa.sh` → micro:tour-en MTE4 (the post-jump skip)
- **First red**: run 35440378070 (micro:tour-en, 11:41Z) — `[bug-P1] TOUR_EN_ESC_SKIP: Escape did not skip the tour (the 'Step 7 of 20' card is still visible)` — after a fully-green battery (mte0 offer, mte1 Next/Back, mte2, mte3's Skip-button unmount + unblock, mte4's Help & Guide menu + the 'Documents & Scanning' section jump to 'Step 7 of 20')
- **Root cause**: the BUG-PD38 mechanism, in a spot PD38 did not touch: the System-Events Escape is not delivered to the WKWebView page's window keydown in this build — from the step-7 scan-view spotlight context it failed (the earlier greens' context delivered it; the P3 key-delivery note stands). MTA0 got the Escape→Skip-button ladder in PD38; mte4 kept the bare Escape contract
- **Fix**: the same BUG-PD38 ladder at mte4 — Escape first, then the card's OWN visible Skip button (the proven affordance — MTE3 used it successfully minutes earlier in the same battery; tour-ar's GREEN proves it mid-tour), then only the honest P1
