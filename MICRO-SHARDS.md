...
| TOUR | `micro:tour-en` | `qa-tour-en` | **implemented** | `micro_tour_en` — the DIRECTIVE §21 walk: the first-login offer (TOUR_GATEWAY=skip keeps it) → Next/Back/progress ('Step 2 of 20' + the step titles) → Skip (overlay unmounts; the plain UI works) → Help & Guide (replay + jump-to-section) → the completion persistence after a logout/login cycle. **Gateway opt-out:** this shard sets `TOUR_GATEWAY=skip` — the offer is the shard's own subject. GREEN (frozen — first flight + the BUG-PD38-era runs) |
| TOUR (Arabic) | `micro:tour-ar` | `qa-tour-ar` | **implemented** | `micro_tour_ar` — MTA0 the English first-login offer (skipped via the BUG-PD38 ladder: Escape → the card's Skip button) → the العربية switch + RTL render → the Help & Guide replay (the ARABIC tour: welcome title, Next/السابق controls, 'الخطوة 1 من 20' progress) → the ArrowRight/ArrowLeft walk → Escape/Skip → the unblocked plain RTL dashboard. `TOUR_GATEWAY=skip` (the offer survives for the shard) |
| LANGUAGE / RTL | `micro:rtl` | `qa-rtl-layout` | **implemented** | `micro_rtl` — the app's RTL layout battery. Keeps the DEFAULT `TOUR_GATEWAY=on`: at the first shell mount the offer is English (default locale), so the gateway dismissal needle works and the battery runs on the plain RTL UI. GREEN (frozen) |
