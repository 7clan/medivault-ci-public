...
echo "[prisma] copying the prisma CLI (self-contained bundle + engines)"
rm -rf "$RES/prisma-cli"
mkdir -p "$RES/prisma-cli/node_modules"
cp -R "$PRISMA_MODULES/prisma" "$RES/prisma-cli/node_modules/prisma"
cp -R "$PRISMA_MODULES/@prisma" "$RES/prisma-cli/node_modules/@prisma"
