 ...
 * Call this during server startup (e.g., in instrumentation.ts or a layout).
 *
...
export function enforceHttpsConfig(): void {
  ...
  const hasExplicitHttps = process.env.HTTPS === 'true' ||
    process.env.NEXT_PUBLIC_BASE_URL?.startsWith('https')

  const isTrustedLocalTlsTermination = process.env.TRUSTED_LOCAL_TLS_TERMINATION === 'true'
